                    ...
                    <div className='w-full h-[80px] flex items-center justify-between p-3'>
                        ...
                        <div className="flex items-center">
                            ...
                            </div>
                            {
                                (user || data) ? (
                                    <div 
                                        className="w-[30px] h-[30px] rounded-full overflow-hidden cursor-pointer hover:opacity-80 transition-opacity" 
                                        onClick={handleProfileClick}
                                        style={{border: activeItem === 5 ? "2px solid #37a39a" : "none" }}
                                    >
                                        <Image 
                                            src={getAvatarUrl(user, data?.user?.image || undefined)} 
                                            alt="User Avatar" 
                                            width={30} 
                                            height={30} 
...

            {
                route === "Verification" && (
                    <>
                        {
                            open && (
                                <CustomModal open={open} setOpen={setOpen} setRoute={setRoute} activeItem={setActive} component={Verification} />
                            )
                        }
                    </>
                )
